...
/**
 * Forge block event helper for placement and removal with events.
 */
public class ForgeBlockEventHelper implements IBlockEventHelper {
    ...
    public boolean placeBlock(Level world, Player player, BlockState state, BlockPos pos, @Nullable BlockItem item) {
        ...
        if (placed) {
            ...
            BlockEvent.EntityPlaceEvent placeEvent = new BlockEvent.EntityPlaceEvent(
                    net.minecraftforge.common.util.BlockSnapshot.create(world.dimension(), world, pos),
